...
coloc.Sgl = function(MyImCl, Plate,Time,Well,Site ,Blue=1,Green=2, Red=3, auto1=T,auto2=T, auto3=T, Cyto = 'Compt 2',Nuc.rm = T, Seg.method = 'Fast', TopSize1 = 29, TopSize2 = 29, TopSize3 = 29,  w1OFF = 0.1, w2OFF = 0.15, w3OFF = 0.1, Nuc.denoising = T,
  ...
  if(TEST){
    ...
    if(Site==1 & writeSeg==T){
      if(getCell==T){
        if(nch>2){ 
          writeImage(paintObjects(NMask,paintObjects(MList[[Green]], paintObjects(MList[[Red]],paintObjects(OMask, rgbImage(blue = EBImage::normalize(IMList[[Blue]],inputRange=Rm1), green=EBImage::normalize(IMList[[Green]], inputRange=Rm2),red =EBImage::normalize(IMList[[Red]], inputRange=Rm3)), col = 'yellow', thick = T),                                                                        
          ...
          writeImage(paintObjects(MList[[Green]], paintObjects(MList[[Red]],paintObjects(OMask, rgbImage(green=EBImage::normalize(IMList[[Green]], inputRange=Rm2),red =EBImage::normalize(IMList[[Red]], inputRange=Rm3)), col = 'yellow', thick = T),                                                                        
        ...
        if(nch>2){ 
          writeImage(paintObjects(T1,paintObjects(MList[[Green]], paintObjects(MList[[Red]],paintObjects(CMask, rgbImage(blue = EBImage::normalize(IMList[[Blue]],inputRange=Rm1), green=EBImage::normalize(IMList[[Green]], inputRange=Rm2),red =EBImage::normalize(IMList[[Red]], inputRange=Rm3)), col = 'yellow', thick = T),                                                                        
                                                                               col = c('red', 'dark red'), opac = c(1,0.3), thick = F), col = c('green', 'darkgreen'), opac = c(1,0.3), thick = F), col = 'blue', thick = T), paste0(path, '/',Plate,'/',Time,'/',Well,'/',Well,'_s',Site,'_ImSeg1.tif'))
        }else{
          writeImage(paintObjects(MList[[Green]], paintObjects(MList[[Red]],paintObjects(CMask, rgbImage(green=EBImage::normalize(IMList[[Green]], inputRange=Rm2),red =EBImage::normalize(IMList[[Red]], inputRange=Rm3)), col = 'yellow', thick = T),                                                                        
                                                               col = c('red', 'dark red'), opac = c(1,0.3), thick = F), col = c('green', 'darkgreen'), opac = c(1,0.3), thick = F), paste0(path, '/',Plate,'/',Time,'/',Well,'/',Well,'_s',Site,'_ImSeg1.tif'))
        }
      }
    }
